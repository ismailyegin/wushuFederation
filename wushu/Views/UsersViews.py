...
def profile_image(request):
    ...
    if not (os.path.isdir('euwuf/media/profile_image')):
        os.makedirs(os.path.join('euwuf/media', 'profile_image'))
    ...
    try:
        ...
        for item in Person.objects.all():
            ...
            dst = os.path.join(BASE_DIR, 'euwuf/media/profile_image/' + item.pasaport+extension)
            if not Path(dst).is_file():
                shutil.copyfile(item.profileImage.path, dst)
        zip_file=None
        is_file=Path(os.path.join(BASE_DIR, 'euwuf/media/profile_image.zip'))
        if not is_file.is_file():
            zip_file=shutil.make_archive(MEDIA_ROOT, 'zip', MEDIA_ROOT)
        else:
            zip_file=os.path.join(BASE_DIR, 'euwuf/media//profile_image.zip')
